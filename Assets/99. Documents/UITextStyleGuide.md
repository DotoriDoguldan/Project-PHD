...
현재 씬이 이 규격을 지키는지는 **PHD ▸ UI ▸ 9. 규격 점검**으로 확인한다.

## 적용 범위

`PixelPerfectCamera`(270×480 아트 픽셀, PPU 32) + `PixelPerfectUIScaler` 가 붙은 캔버스에 적용한다.
이 구성에서 **캔버스 1 단위 = 아트 1 픽셀**이고, 아트 픽셀은 정수 배율로만 확대된다.
따라서 TMP `Font Size` 값은 그대로 아트 픽셀 크기다.
...
**두 단계만 쓴다.** 표에 없는 값을 쓰지 않는다. 상수는 `GameLayout` 에 있다.

| 토큰 | 크기 | 캡 | 행 | 역할 |
|---|---|---|---|---|
| `TextBase` | **14** | 13px | 23px | 수치 · 라벨 · 버튼 · 안내 — 사실상 전부 |
| `TextDisplay` | **24** | 22px | 39px | 팝업 제목 |
...
- 지금 쓰는 문구: `TAP TO START`, `Touch to Start`, `With CORTIS`, `score`, `round`,
  `YOUR TURN`, `PERFECT +10`, `GAME OVER`, `NEW BEST`, `WRONG!`, `RESULT`, `RETRY`, `CLOSE`, `PLAY`
...
1. `PHD ▸ UI ▸ 8. 폰트 문자셋 뽑기` — 콘솔에 문자셋이 찍힌다.
   씬과 공용 프리팹을 읽어 ASCII 94자 + 실제로 쓰인 ASCII 밖 문자를 모아준다.
