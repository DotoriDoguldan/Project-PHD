...
**카카오톡은 한국어에서만 나간다.** 카카오톡은 사실상 한국에서만 쓰는 앱이라, 다른 언어에서는
`Btn_Kakao` 를 끄고 `Btn_Share` 를 `Area_Share` 한가운데(x 0)로 옮긴다. DOM 쪽도 같은 판단을
payload(`kakao`)로 받아 카카오 버튼을 감추고 `#phd-share-layer.is-solo` 로 가운데 정렬한다 —
**한쪽만 빠지면 눌리지 않는 아이콘이나 아이콘 없는 유령 버튼이 남는다.** 판단은
`WebShare.UsesKakao` 한 곳에서만 하고, `WebShareAnchor` 가 Unity 아이콘과 payload 를 함께 맞춘다.

**공유 글도 게임에서 고른 언어로 나간다.** 문장 조립은 `PHDShare` 가 하지만, 언어를 따라
바뀌는 조각(신기록 문구·카카오 마지막 줄·안내 토스트·버튼 이름)은 전부 `GameText` 표에서 꺼내
`WebShare.Show` 의 payload(`text`)에 실어 넘긴다 — **문구를 고치는 곳은 여전히 `GameText` 하나다.**
`ROUND` · `SCORE` · `BEST` 처럼 어느 언어에서나 같은 머리글만 템플릿이 직접 찍는다.
템플릿에 남은 한국어는 payload 가 없을 때(콘솔 호출·구버전 빌드) 쓰는 기본값과,
카카오 전용 문구(버튼 이름·마지막 줄·토스트)다 — 그 버튼이 한국어에서만 나오므로 번역할 것이 없다.
